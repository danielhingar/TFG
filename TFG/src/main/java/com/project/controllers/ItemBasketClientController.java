...
public class ItemBasketClientController {
	...
	// ------------------------Create a item of the basket-----------------------------------
		@PostMapping("/create/{idProduct}")
		@ResponseStatus(HttpStatus.CREATED)
		public ResponseEntity<?> create(@Valid @RequestBody ItemBasket itemBasket, BindingResult bindingResult,@PathVariable int idProduct) {
		...
		public ResponseEntity<?> update(@Valid @RequestBody ItemBasket itemBasket, BindingResult bindingResult,
			...
			if (itemBasketActually == null) {
				...
				response.put("mensaje",
						"Error: no se pudo editar el item,".concat(id1.toString().concat(" no existe ")));
				return new ResponseEntity<Map<String, Object>>(response, HttpStatus.NOT_FOUND);
			}
